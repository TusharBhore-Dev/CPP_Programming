...
/*
    ...
    START
        ...
        Display an inverted right-angled numeric triangle using nested for loops
    STOP
*/
...
//
//  Class Name : Pattern
//  Description : It is used to display the inverted triangular numeric pattern
...
class Pattern
{
    ...
    int iRow = 0 , iCol = 0 , i = 0 , j = 0;
            ...
            void DisplayPattern()
            {
                ...
                for( i = 1; i <= iRow; i++ )
                {
                    // Inner loop handles printing numbers, decreasing count per row
                    // Formula: (iCol + 1) - i calculates decreasing column limit
                    for( j = 1; j <= ( ( iCol + 1 ) - i ); j++ )
                    {
                        // Printing the current column number
                        cout<<j<<"\t";
                    }
